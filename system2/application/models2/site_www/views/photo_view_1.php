<script type="text/javascript">
$(document).ready(function(){
  caseShowName("example_list",-210,100);
  searchTextClear("search_text","请输入企业或网站名称");
  moreCartoon("content_more",-200,200);
  //hoverCartoon("content_hover",-200,200);
  cartoonNav("mainnav","navsel",90,2);
  navScroll("navbox","go_topnav");
});
</script>
<link href="<?php echo $base_url;?>resource/site_yn/css/cartoon.css" rel="stylesheet" type="text/css" />
	<!-- 主体 -->
<!--banner-->
<section class="content_banner" style="background:#00aae8;">
 <div class="main">
   <a href="/case/"><img src="<?php echo $base_url;?>resource/site_yn/images/content03.jpg" /></a>
 </div>
  <div class="clear"></div>
</section><!--banner END-->
<section class="contentbox04">
  <div class="content_main"> 
    <nav class="example_nav">
<?php 
$i=1;
foreach ($category_list as $row){
$class=($category==$row->id) ? "sel" : "";?>
<a class="a<?php echo $i;?> <?php echo $class;?>"  title="<?php echo $row->name;?>" href="<?php echo $site_url;?>photo/index/category_<?php echo $row->id;?>"><span><?php echo msubstr($row->name,0,8);?></span></a>
<?php $i++; }?>

<script type="text/javascript">
function formCheck()
{
        var $q=$("#keyword").val();
		if ($.trim($q)=='' || $.trim($q)=='请输入企业或网站名称'){
		showtip("亲~请输入关键词查询！");
		$("#keyword").focus();
		return false;
		}
}
</script>
      <div class="search_box example_search"> 
      <form name="search" type="get" action="<?php echo $site_url;?>photo" id="search" onsubmit="return formCheck();">
        <input type="submit" class="sub" value="案例搜索" />
      <input type="text" class="text search_text" value=""  name="keyword" id="keyword"/>
      </form>
      <div class="clear"></div>
      </div>
      <div class="clear"></div>
    </nav><!--example_nav END-->
    <div class="main">
  <?php 
	$i=1;
	foreach ($list["this_data"] as $row){
		$class=($i % 3==0) ? "example_list_0" : "";?>
<a class="example_list <?php echo $class;?>" title="<?php echo $row-><field_title>;?>" href="<?php echo $site_url;?>photo/show/<?php echo $row->id;?>">
        <img src="<?php echo (@$row-><field_pic>) ? ((substr(@$row-><field_pic>,0,4)=='http' || substr(@$row-><field_pic>,0,1)=='/') ? @$row-><field_pic> : $base_url.@$row-><field_pic>)  : "";?>" width="348" height="210"/>
        <span><p></p><?php echo $row-><field_title>;?></span>
      </a>
      <?php $i++;}?>
      <div class="clear"></div>
      <div class="content_page example_page">
      <menu>
      <div> <?php echo $list['this_page'];?></div>
      <div class="clear"></div>
      </menu>
      <div class="clear"></div>
      </div>
    </div>
    <div class="clear"></div>
	    <table class="content_botbox">
      <tr>
        <td><a class="content_bot_a a1" href="http://wpa.b.qq.com/cgi/wpa.php?ln=1&key=XzkzODA1OTU5MF8yMjg2MzRfNDAwNzM5MTIwMF8yXw" target="_blank" rel="nofollow"></a></td>
        <td><a class="content_bot_a a2"  href="javascript:loginWinOpen('weixin_win','myselfbox',200);"></a></td>
        <td rowspan="2">
          <a class="content_more"  href="/about/video/">
            <span><img src="<?php echo $base_url;?>resource/site_yn/images/arrow02.png" /></span>
            <p>视频播放</p>
          </a>
        </td>
        <td rowspan="2" class="text">
          <p><img src="<?php echo $base_url;?>resource/site_yn/images/content_bot05.png" /></p>
          <p>地址：云南省.昆明市环城南路668号（云纺商业区）云纺国际商厦A座18层-19层</p>
          <p>电话：0871-63167196、63157171、63157172、63157517、63168206、63156299</p>
          <p>咨询：0871-63395062、售后：0871-63365932、投诉：0871-63386022</p>
          <p>传真：0871-63365715、邮箱：info@ynyes.com</p>
        </td>
      </tr>
      <tr>
        <td><a class="content_bot_a a3" href="http://weibo.com/ynsite" target="_blank" rel="nofollow"></a></td>
        <td><a class="content_bot_a a4" href="/about/service/"></a></td>
      </tr>
    </table><!--content_botbox END-->
  </div><!--main END-->
  <div class="clear"></div>
  <div class="content_bot"></div><!--content_bot END-->
</section><!-- index_part END-->
	<!-- /主体 -->